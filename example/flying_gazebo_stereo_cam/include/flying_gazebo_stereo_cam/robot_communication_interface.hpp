...
class CommunicationInterface
    : public ig_active_reconstruction::robot::CommunicationInterface {
public:
...
public:
  /*! Constructor
   * @param nh Sets the ros node handle used for data subscription and
   * publication.
   * @param controller Sets the controller of the camera that will be moved.
   * @param in_name Input name (Where pcl input is retrieved, relative to node
   * namespace)
   * @param out_name How outputs (srv/topic) are advertised (relative to node
   * namespace)
   */
  ...
  */
  virtual MovementCost movementCost(View &start_view, View &target_view,
...
private:
  ...
  ros_tools::PclRerouter pcl_rerouter_; //! Since the gazebo stereo camera
