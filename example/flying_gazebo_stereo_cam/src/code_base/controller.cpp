...
Controller::Controller(std::string cam_model_name)
    : cam_model_name_(cam_model_name), has_moved_(false),
      keepPublishing_(false), cam_to_image_(0.5, 0.5, -0.5, 0.5) {}
...
movements::Pose Controller::currentPose() {
  ...
  if (!response)
    throw std::runtime_error("flying_gazebo_stereo_cam::Controller::"
...
void Controller::startTfPublisher(std::string camera_frame_name,
                                  std::string world_frame_name) {
  keepPublishing_ = true;
  publisher_ = std::thread(&Controller::keepPublishing, this, camera_frame_name,
                           world_frame_name);
}

void Controller::stopTfPublisher() { keepPublishing_ = false; }

void Controller::keepPublishing(std::string camera_frame_name,
                                std::string world_frame_name) {
  while (keepPublishing_) {
    if (true || has_moved_) {
      ...
      tf_broadcaster_.sendTransform(tf::StampedTransform(
          cam_2_origin, ros::Time::now(), world_frame_name, camera_frame_name));
    }
