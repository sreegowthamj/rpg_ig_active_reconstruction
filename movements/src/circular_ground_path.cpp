...
double CircularGroundPath::totalAngle(movements::Pose &_center)
{
        ...
        Eigen::Vector2d start_local =
                RelativePositionCalculator::localCoordinates(_center.position,
                                                             start_point_);
        ...
        double phi_end_localrot = angles::normalize_angle_positive(
                phi_end_local - phi_start_local);
...
CircularGroundPath::RelativePositionCalculator::RelativePositionCalculator(
    ...
    : start_point_(_start_point), end_point_(_target_point),
      angular_speed_(fabs(_angular_speed)), direction_(_direction), time_(_time)
{
}
        ...
        movements::Pose const &_path_center)
{
        ...
        double radial_distance_to_move =
                center2end.norm() - center2start.norm();

        double total_time_for_execution = total_angle_to_move / angular_speed_;
        double radial_speed =
                radial_distance_to_move / total_time_for_execution;
        ...
        if (time_ >= total_time_for_execution) {
                out_point_local = end_local;
                   ...
                   <= 0) // cases could be sped up by doing this test at the
        ...
        } else // point on path
        {
                ...
                out_point_local =
                        (start_local.norm() + radial_speed * time_)
                        * Eigen::Vector2d(cos(out_angle), sin(out_angle));
        }

        Eigen::Vector3d out_point(
                out_point_local.x() + _path_center.position.x(),
