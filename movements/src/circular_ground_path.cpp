...
CircularGroundPath::CircularGroundPath(Eigen::Vector3d _start_point,
    ...
    : start_point_(_start_point.x(), _start_point.y()),
      ...
      angular_speed_(_angular_speed), direction_(_direction) {}

CircularGroundPath::CircularGroundPath(movements::Pose _start_point,
                                       movements::Pose _target_point,
                                       double _angular_speed,
                                       MovementDirection _direction)
...
CircularGroundPath::create(movements::Pose _start_point,
                           movements::Pose _target_point, double _angular_speed,
                           MovementDirection _direction) {
  return KinematicMovementDescription(new CircularGroundPath(
      _start_point, _target_point, _angular_speed, _direction));
}
...
double CircularGroundPath::totalAngle(movements::Pose &_center) {
  if (_center.position.x() == start_point_.x() &&
          _center.position.y() == start_point_.y() ||
  ...
  Eigen::Vector2d start_local = RelativePositionCalculator::localCoordinates(
      _center.position, start_point_);
  Eigen::Vector2d end_local = RelativePositionCalculator::localCoordinates(
      _center.position, end_point_);
  ...
  double phi_end_localrot =
      angles::normalize_angle_positive(phi_end_local - phi_start_local);
...
std::string CircularGroundPath::type() { return "CircularGroundPath"; }

RelativeMovement CircularGroundPath::operator()(double _time) {
  return RelativeMovement(new RelativePositionCalculator(
...
CircularGroundPath::RelativePositionCalculator::RelativePositionCalculator(
    ...
    : start_point_(_start_point), end_point_(_target_point),
      angular_speed_(fabs(_angular_speed)), direction_(_direction),
      time_(_time) {}

movements::Pose CircularGroundPath::RelativePositionCalculator::applyToBasePose(
    movements::Pose const &_path_center) {
  if (_path_center.position.x() == start_point_.x() &&
          _path_center.position.y() == start_point_.y() ||
  ...
  Eigen::Vector2d start_local =
      localCoordinates(_path_center.position, start_point_);
  Eigen::Vector2d end_local =
      localCoordinates(_path_center.position, end_point_);
  ...
  } else if (time_ <= 0) // cases could be sped up by doing this test at the
                         // beginning, however this isn't considered a regular
                         // case and it would make the code look messy
  {
  ...
  } else // point on path
  {
    ...
    out_point_local = (start_local.norm() + radial_speed * time_) *
                      Eigen::Vector2d(cos(out_angle), sin(out_angle));
  }

  Eigen::Vector3d out_point(out_point_local.x() + _path_center.position.x(),
                            out_point_local.y() + _path_center.position.y(),
                            _path_center.position.z());
  ...
  Eigen::Quaterniond out_orientation(
      cos(orientation_angle_half), 0, 0,
      sin(orientation_angle_half)); // Quaternion(w,x,y,z)
...
Eigen::Vector2d
CircularGroundPath::RelativePositionCalculator::localCoordinates(
    Eigen::Vector3d const &_center, Eigen::Vector2d global_coordinates) {
