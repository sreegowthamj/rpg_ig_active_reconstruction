...
CombinedRelativeMovement CombinedKinematicMovementDescription::
operator()(double _time) {
...
std::vector<CombinedRelativeMovement>
CombinedKinematicMovementDescription::relativePath(double _start_time,
                                                   double _end_time,
                                                   double _step_size) {
...
CombinedKinematicMovementDescription::path(movements::Pose _base_pose,
...
operator=(CombinedRelativeMovement const &_to_equal) {
  ...
  BOOST_FOREACH (auto rel_movement, _to_equal.relative_movement_queue_) {
    relative_movement_queue_.push_back(std::pair<int, RelativeMovement>(
        relative_movement_queue_.size(), rel_movement));
  }
...
CombinedKinematicMovementDescription &CombinedKinematicMovementDescription::
operator=(RelativeMovement const &_to_equal) {
  relative_movement_queue_.clear();
  kinematic_movement_queue_.clear();

  relative_movement_queue_.push_back(
      std::pair<int, RelativeMovement>(0, _to_equal));

  return *this;
}

CombinedKinematicMovementDescription &CombinedKinematicMovementDescription::
operator=(KinematicMovementDescription const &_to_equal) {
  relative_movement_queue_.clear();
  kinematic_movement_queue_.clear();

  kinematic_movement_queue_.push_back(
      std::pair<int, KinematicMovementDescription>(0, _to_equal));
...
operator+=(CombinedRelativeMovement const &_to_add) {
  BOOST_FOREACH (auto rel_movement, _to_add.relative_movement_queue_) {
    relative_movement_queue_.push_back(
        std::pair<int, RelativeMovement>(nrOfMovements(), rel_movement));
  }
  return *this;
}
...
operator+=(RelativeMovement const &_to_add) {
  relative_movement_queue_.push_back(
      std::pair<int, RelativeMovement>(nrOfMovements(), _to_add));
  return *this;
}
...
operator+=(KinematicMovementDescription const &_to_add) {
  kinematic_movement_queue_.push_back(
      std::pair<int, KinematicMovementDescription>(nrOfMovements(), _to_add));
  return *this;
}
...
operator+=(CombinedKinematicMovementDescription const &_to_add) {
  ...
  BOOST_FOREACH (auto rel_movement, _to_add.relative_movement_queue_) {
    relative_movement_queue_.push_back(std::pair<int, RelativeMovement>(
        old_size + rel_movement.first, rel_movement.second));
  }
  BOOST_FOREACH (auto kin_movement, _to_add.kinematic_movement_queue_) {
    kinematic_movement_queue_.push_back(
        std::pair<int, KinematicMovementDescription>(
            old_size + kin_movement.first, kin_movement.second));
  }
  return *this;
}
