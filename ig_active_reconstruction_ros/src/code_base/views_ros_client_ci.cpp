...
{
        planning_space_receiver_ = nh.serviceClient<
                ig_active_reconstruction_msgs::ViewSpaceRequest>("views/space");
        views_adder_ = nh.serviceClient<
                ig_active_reconstruction_msgs::ViewSpaceUpdate>("views/add");
        views_deleter_ =
                nh.serviceClient<ig_active_reconstruction_msgs::DeleteViews>(
                        "views/delete");
}
...
const ViewSpace &RosClientCI::getViewSpace()
{
        ...
        if (response)
                viewspace_ = ros_conversions::viewSpaceFromMsg(
                        call.response.viewspace);

        return viewspace_;
}

RosClientCI::ViewSpaceUpdateResult
RosClientCI::addViews(std::vector<View> &new_views)
{
...
RosClientCI::ViewSpaceUpdateResult
RosClientCI::deleteViews(std::vector<View::IdType> &view_ids)
{
