...
RosClientCI::RosClientCI(ros::NodeHandle nh_sub) : nh_sub_(nh_sub) {
  current_view_retriever_ =
      nh_sub_.serviceClient<ig_active_reconstruction_msgs::ViewRequest>(
          "robot/current_view");
  data_retriever_ =
      nh_sub_.serviceClient<ig_active_reconstruction_msgs::RetrieveData>(
          "robot/retrieve_data");
  cost_retriever_ = nh_sub_.serviceClient<
      ig_active_reconstruction_msgs::MovementCostCalculation>(
      "robot/movement_cost");
  robot_mover_ =
      nh_sub_.serviceClient<ig_active_reconstruction_msgs::MoveToOrder>(
          "robot/move_to");
}
...
MovementCost RosClientCI::movementCost(views::View &target_view) {
  ...
  MovementCost cost =
      ros_conversions::movementCostFromMsg(request.response.movement_cost);
  if (!response) {
