...
{
        viewspace_service_ = nh.advertiseService(
                "views/space", &RosServerCI::viewspaceService, this);
        views_adder_service_ = nh.advertiseService(
                "views/add", &RosServerCI::viewsAdderService, this);
        views_deleter_service_ = nh.advertiseService(
                "views/delete", &RosServerCI::viewsDeleterService, this);
}
...
const ViewSpace &RosServerCI::getViewSpace()
{
        if (linked_interface_ == nullptr)
                throw std::runtime_error(
                        "views::RosServerCI::Interface not linked.");

        return linked_interface_->getViewSpace();
}

RosServerCI::ViewSpaceUpdateResult
RosServerCI::addViews(std::vector<View> &new_views)
{
        if (linked_interface_ == nullptr)
                throw std::runtime_error(
                        "views::RosServerCI::Interface not linked.");
...
RosServerCI::ViewSpaceUpdateResult
RosServerCI::deleteViews(std::vector<View::IdType> &view_ids)
{
        if (linked_interface_ == nullptr)
                throw std::runtime_error(
                        "views::RosServerCI::Interface not linked.");
        ...
        ig_active_reconstruction_msgs::ViewSpaceRequest::Response &res)
{
        ...
        if (linked_interface_ == nullptr) {
                ...
                res.viewspace_status =
                        ros_conversions::viewSpaceStatusToMsg(status);
                return true;
        }
        ...
        ig_active_reconstruction_msgs::ViewSpaceUpdate::Response &res)
{
        ...
        if (linked_interface_ == nullptr) {
                ViewSpaceUpdateResult result =
                        ViewSpaceUpdateResult::NOT_AVAILABLE;
                res.update_result =
                        ros_conversions::viewSpaceUpdateResultToMsg(result);
                return true;
        }
        ...
        ig_active_reconstruction_msgs::DeleteViews::Response &res)
{
        ...
        ViewSpaceUpdateResult result =
                linked_interface_->deleteViews(delete_ids);
        res.update_result = ros_conversions::viewSpaceUpdateResultToMsg(result);
