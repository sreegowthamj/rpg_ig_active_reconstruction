...
{
TEMPT
CSCOPE::RosServerCI(ros::NodeHandle nh,
                    POINTER_TYPE<CommunicationInterface> linked_interface)
    : nh_(nh), linked_interface_(linked_interface)
{
        view_ig_computation_ = nh.advertiseService(
                "world/information_gain", &CSCOPE::igComputationService, this);
        map_metric_computation_ = nh.advertiseService(
                "world/map_metric", &CSCOPE::mmComputationService, this);
        available_ig_receiver_ = nh.advertiseService(
                "world/ig_list", &CSCOPE::availableIgService, this);
        available_mm_receiver_ = nh.advertiseService(
                "world/mm_list", &CSCOPE::availableMmService, this);
}

TEMPT
typename CSCOPE::ResultInformation
CSCOPE::computeViewIg(IgRetrievalCommand &command, ViewIgResult &output_ig)
{
        if (linked_interface_ == NULL)
                throw std::runtime_error(
                        "world_representation::CSCOPE::Interface not linked.");
...
TEMPT
bool CSCOPE::igComputationService(
        ig_active_reconstruction_msgs::InformationGainCalculation::Request &req,
        ig_active_reconstruction_msgs::InformationGainCalculation::Response
                &res)
{
        ...
        if (linked_interface_ == NULL) {
                ...
                failed.status =
                        ros_conversions::resultInformationToMsg(failed_status);
                unsigned int number_of_metrics =
                        (!req.command.metric_ids.empty())
                                ? req.command.metric_ids.size()
                                : req.command.metric_names.size();
        ...
        ViewIgResult result;
        IgRetrievalCommand command =
                ros_conversions::igRetrievalCommandFromMsg(req.command);
        linked_interface_->computeViewIg(command, result);

        BOOST_FOREACH (IgRetrievalResult &ig_res, result) {
                res.expected_information.push_back(
                        ros_conversions::igRetrievalResultToMsg(ig_res));
        }
        return true;
}

TEMPT
bool CSCOPE::mmComputationService(
        ig_active_reconstruction_msgs::MapMetricCalculation::Request &req,
        ig_active_reconstruction_msgs::MapMetricCalculation::Response &res)
{
        ...
        BOOST_FOREACH (MapMetricRetrievalResult &ig_res, result) {
                ...
                gain.status =
                        ros_conversions::resultInformationToMsg(ig_res.status);
...
TEMPT
bool CSCOPE::availableIgService(
        ig_active_reconstruction_msgs::StringList::Request &req,
        ig_active_reconstruction_msgs::StringList::Response &res)
{
...
TEMPT
bool CSCOPE::availableMmService(
        ig_active_reconstruction_msgs::StringList::Request &req,
        ig_active_reconstruction_msgs::StringList::Response &res)
{
