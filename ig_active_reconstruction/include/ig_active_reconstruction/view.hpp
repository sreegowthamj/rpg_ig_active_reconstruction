...
class View
{
      ...
      public:
        ...
        /*! Constructor with given id. (This is for communication purposes, if
         * the view already exists and should keep its id).
         */
        View(IdType id);
      ...
      private:
        IdType index_; //! Unique view index per process, increased by one each
                       //! time a new view is created.
        static IdType
                runningIndex_; //! The index of the next newly created view.

        movements::Pose pose_;     //! the pose...
        std::string source_frame_; //! pose is relative to this frame, that is
                                   //! the transform that could be used to
                                   //! transform a point in the pose frame into
                                   //! one in the source frame
        boost::shared_ptr<ViewInfo>
                associated_data_; //! interface that gives possibility for a
                                  //! robot to store robot specific data that is
                                  //! associated with the pose, e.g. for the
                                  //! Youbot some IK calculations can be made
                                  //! beforehand but the robot needs to know
                                  //! which data implements a given view
        ...
        bool is_bad_;       //! marks a bad view for whatever reason (e.g. data
                            //! reception failed here or the like)
