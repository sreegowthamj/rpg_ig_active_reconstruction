...
public:
  ...
  /*! Sets the robot communication interface with which the utility function
   * corresponds.
   */
...
protected:
   ...
   * @param ig_vector (output) Vector in which the ig values will be set, must
   * already have correct size
   * @param total_ig (output) total information gain calculated within this
   * function
   ...
   * function [0-(batch_size-1)]. All id's are separated into batches of size
   * batch_size. This function will process every base_index-th of it. E.g.
   * batch_size = 3, base_index=2: The function will process the 2th, 5th, 8th,
   * 11th, etc entry...
