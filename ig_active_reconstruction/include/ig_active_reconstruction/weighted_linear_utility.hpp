 ...
 * a linear, but weighted combination, each normalized over the total ig and
 * cost for all views respectively.
 *
 * IG retrieval is in this implementation not multithreaded and thus takes some
 * time.
 */
class WeightedLinearUtility : public UtilityCalculator
{
      public:
        /*! Constructor
         * @param cost_weight Overall cost weight in the equation compared to
         * information gains.
         */
         ...
         */
        virtual void setIgRetrievalConfig(
                world_representation::CommunicationInterface::IgRetrievalConfig
                        &config);

        /*! Sets the world representation communication interface with which the
         * utility function corresponds.
         */
        virtual void setWorldCommUnit(
                boost::shared_ptr<world_representation::CommunicationInterface>
                        world_comm_unit);

        /*! Sets the robot communication interface with which the utility
      ...
      protected:
         ...
         * @param ig_vector (output) Vector in which the ig values will be set,
         * must already have correct size
         * @param total_ig (output) total information gain calculated within
         * this function
         * @param command Prebuilt command structure, only lacking the path
         * entry
         * @param id_set Set of views for which getNbv was called.
         * @param viewspace Corresponding viewspace
         * @param base_index Which entry within the batch is processed by this
         * function [0-(batch_size-1)]. All id's are separated into batches of
         * size batch_size. This function will process every base_index-th of
         * it. E.g. batch_size = 3, base_index=2: The function will process the
         * 2th, 5th, 8th, 11th, etc entry...
         * @param batch_size Size of the batch, matches the number of spawned
         * threads.
         */
        void
        getIg(std::vector<double> &ig_vector, double &total_ig,
      ...
      protected:
        boost::shared_ptr<world_representation::CommunicationInterface>
                world_comm_unit_; //! Interface to world representation.
        boost::shared_ptr<robot::CommunicationInterface>
                robot_comm_unit_; //! Interface to robot.

        world_representation::CommunicationInterface::IgRetrievalConfig
                ig_retrieval_config_; //! Will be used for ig retrieval.
