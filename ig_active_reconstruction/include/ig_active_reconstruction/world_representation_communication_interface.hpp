...
{
      public:
        /*! Returned status for a specific ig metric calculation, to be treated
         * as scoped enum. Scoped enum variant for cpp03...
         */
        struct ResultInformation {
              public:
                enum Enum {
                        SUCCEEDED = 0, //! IG was successfully calculated.
                        FAILED,        //! IG could not be calculated, an error
                                       //! occured.
                        UNKNOWN_METRIC //! The IG name was unknown and hence the
                                       //! IG could not be calculated
                } res;
              ...
              public:
                ...
                bool operator==(const Enum comp) const
                {
                        return res == comp;
                };
                bool operator!=(const Enum comp) const
        ...
        struct IgRetrievalResult {
                ...
                double predicted_gain;    //! Calculated information gain if the
                                       //! call succeeded, undefined otherwise.
        };
        ...
        struct IgRetrievalConfig {
              public:
                ...
                struct SubWindow {
                        double min_x_perc,
                                max_x_perc; //! x-coordinates-window through
                                            //! which rays will be cast.
                                            //! [percentage of image width,
                                            //! 0.0-1.0]
                        double min_y_perc,
                                max_y_perc; //! y-coordinates-window through
                                            //! which rays will be cast.
              ...
              public:
                double ray_resolution_x; //! How many rays are cast per pixel on
                                         //! the image's x-axis to obtain the
                                         //! information. [rays/px] Default: 1.0
                double ray_resolution_y; //! How many rays are cast per pixel on
                                         //! the image's y-axis to obtain the
                                         //! information. [rays/px] Default: 1.0

                SubWindow
                        ray_window; //! Defines a subwindow of the image on
                                    //! which the rays shall be cast. Defaults
                                    //! to the complete window. Default:
                                    //! [0.0, 1.0] for both x- and y-coordinate
                                    //! windows, ie the complete image.

                double max_ray_depth; //! Maximal ray depth for the ig
                                      //! computation. [World representation
                                      //! units, usually m] Default: 10.0
        };

        /*! Command structure for information gain retrieval computation. The
         * struct features a constructor that sets all members to default
         * values. See member descriptions for details.
         */
        ...
        struct MapMetricRetrievalResult {
                ResultInformation status; //! Status.
                double value; //! Calculated information gain if the call
                              //! succeeded, undefined otherwise.
        };

        typedef std::vector<MapMetricRetrievalResult>
                MapMetricRetrievalResultSet;
      ...
      public:
         ...
         * @param command Specifies which information gains have to be
         * calculated and for which pose along with further parameters that
         * define how the ig('s) will be collected.
         * @param output_ig (Output) Vector with the results of the information
         * gain calculation. The indices correspond to the indices of the names
         * in the metric_names array within the passed command.
         */
