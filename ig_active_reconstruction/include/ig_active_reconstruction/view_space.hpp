 ...
 *
 * TODO: Change internal implementation details to speed up insertions and
 * deletions.
 */
class ViewSpace {
public:
  ...
  class ConstIterator; // forward declaration for bidirectional iterator type to
                       // const View
...
public:
  ...
  /*! returns indexes of all view points in the view space as a vector that are
   * reachable, are not "bad" and (optionally) have never been visited.
   * @param ignore_visited whether already visited views are left out of the
   * "good viewspace" or not
   */
   ...
   * @return True if all views were found (and deleted). If it is false, some
   * may still have been deleted.
   */
   ...
   * @return How many times the view has been visited. If the view id is
  ...
  /*! returns the view closest to the position passed, ignoring orientation. If
   * more than one views in the view space have the same distance, the first
   * found is returned
   ...
   */
  void
  getViewsInRange(View &reference_view, double distance,
   ...
   *
   * Format (first number of views in the file, then each view represented by
   * its position and a quaternion for its orientation):
   * Nr_of_views
   * pos_1.x pos_1.y pos_1.z orientation_1.x orientation_1.y
   * orientation_1.zorientation_1.w
   * pos_2.x pos_2.y pos_2.z orientation_2.x orientation_2.y orientation_2.z
   * orientation_2.w
   * (...)
   *
 ...
 */
class ViewSpace::Iterator
    : public std::iterator<std::bidirectional_iterator_tag, View> {
public:
