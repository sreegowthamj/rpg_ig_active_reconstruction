...
class BasicViewPlanner
{
      public:
        enum struct Status {
                UNINITIALIZED, // Still waiting for necessary data, can't be run
                               // yet
        ...
        struct Config {
              ...
              public:
                bool discard_visited; //! Whether views should be discarded once
                                      //! visited. Default: false.
                int max_visits; //! Maximal number a view can be visited before
                                //! it is discarded, -1 = infinite. Default: -1.
        };
      ...
      public:
        ...
        /*! Sets the viewspace communication interface with which the view
         * planner corresponds. Can't be set if running.
         */
        virtual void
        setViewsCommUnit(boost::shared_ptr<views::CommunicationInterface>
                                 views_comm_unit);

        /*! Sets the world representation communication interface with which the
         * view planner corresponds. Can't be set if running.
         */
        ...
        /*! Sets a goal evaluation module which determines if the view planner
         * shall continue or not. Can't be set if running.
         */
        virtual void setGoalEvaluationModule(
                boost::shared_ptr<GoalEvaluationModule> goal_evaluation_module);

        /*! Starts the procedure in its own thread if it was stopped, continues
         * the procedure if it was paused.
         * @return True if the procedure started successfully, false if not
         * (e.g. because no all necessary parameters are set, like the
         * communication units)
         */
        virtual bool run();

        /*! Pauses the procedure if it is running, does nothing otherwise.
         * The call returns immediately, even though the procedure continues
         ...
         * The call returns immediately, even though the procedure continues
         * until it reaches an exit point.
         */
        virtual void stop();
      ...
      protected:
        ...
        boost::shared_ptr<robot::CommunicationInterface>
                robot_comm_unit_; //! Robot communication interface with which
                                  //! the view planner corresponds.
        boost::shared_ptr<views::CommunicationInterface>
                views_comm_unit_; //! Viewspace communication interface with
                                  //! which the view planner corresponds.
        boost::shared_ptr<world_representation::CommunicationInterface>
                world_comm_unit_; //! World representation communication
                                  //! interface with which the view planner
                                  //! corresponds.
        ...
        bool runProcedure_;   //! True as long as the procedure is running or
                              //! paused.
