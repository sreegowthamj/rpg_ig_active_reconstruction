...
void ViewSpace::getGoodViewSpace(IdSet &out, bool ignore_visited) {
  for (auto &pair : views_index_map_) {
    ...
    if (view.reachable() && (view.timesVisited() == 0 || !ignore_visited) &&
        !view.bad()) {
...
View ViewSpace::getView(View::IdType index) {
  try {
    ...
    throw std::out_of_range(
        "ViewSpace::getView: the given index is out of range");
  }
}
...
void ViewSpace::push_back(View new_vp) {
  ...
  views_index_map_[new_vp.index()] =
      new_vp; // std::reference_wrapper<View>(view_ref);
}

View ViewSpace::getAClosestNeighbour(View &_view) {
  if (views_index_map_.empty())
    throw std::runtime_error("ViewSpace::getAClosestNeighbour::Cannot find a "
...
unsigned int ViewSpace::size() { return views_index_map_.size(); }
...
void ViewSpace::getViewsInRange(
  ...
  for (auto &pair : views_index_map_) {
    ...
    Eigen::Vector3d dist_vec =
        view.pose().position - _reference_view.pose().position;
...
bool ViewSpace::empty() const { return views_index_map_.empty(); }
...
ViewSpace::Iterator::Iterator(InternalIteratorType it) : it_(it) {}
...
View &ViewSpace::Iterator::operator*() const { return it_->second; }
...
ViewSpace::ConstIterator::ConstIterator(InternalIteratorType it) : it_(it) {}
...
const View &ViewSpace::ConstIterator::operator*() const { return it_->second; }
