...
void ViewSpace::getGoodViewSpace(IdSet &out, bool ignore_visited)
{
        for (auto &pair : views_index_map_) {
                ...
                if (view.reachable()
                    && (view.timesVisited() == 0 || !ignore_visited)
                    && !view.bad()) {
...
{
        try {
                return static_cast<View &>(views_index_map_.at(index))
                        .timesVisited();
        } catch (...) {
                return 0;
        }
...
{
        try {
                static_cast<View &>(views_index_map_.at(index))
                        .timesVisited() += 1;
                return;
        } catch (...) {
                return;
...
{
        try {
                static_cast<View &>(views_index_map_.at(index)).reachable() =
                        false;
                return;
        } catch (...) {
                return;
...
void ViewSpace::loadFromFile(std::string _filename)
{
        ...
        for (unsigned int i = 0; i < nr_of_views; ++i) {
                ...
                views_index_map_[new_pose.index()] =
                        new_pose; // view_space_.back();
        }
}
