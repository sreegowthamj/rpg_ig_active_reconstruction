...
PinholeCamRayCaster::ResolutionSettings::ResolutionSettings()
    : ray_resolution_x(1.0), ray_resolution_y(1.0), min_x_perc(0),
      min_y_perc(0), max_x_perc(1), max_y_perc(1)
{
}

bool PinholeCamRayCaster::ResolutionSettings::
operator!=(ResolutionSettings &comp) const
{
        return ray_resolution_x != comp.ray_resolution_x
               || ray_resolution_y != comp.ray_resolution_y
               || min_x_perc != comp.min_x_perc || min_y_perc != comp.min_y_perc
...
PinholeCamRayCaster::PinholeCamRayCaster(Config config)
    : config_(config), ray_directions_(boost::make_shared<RayDirectionSet>())
{
...
PinholeCamRayCaster::RayDirection
PinholeCamRayCaster::projectPixelTo3dRay(unsigned int x_px, unsigned int y_px)
{
        RayDirection dir;

        dir(0) = (x_px - config_.camera_matrix(0, 2))
                 / config_.camera_matrix(0, 0);
        dir(1) = (y_px - config_.camera_matrix(1, 2))
                 / config_.camera_matrix(1, 1);
        dir(2) = 1.0;
...
boost::shared_ptr<PinholeCamRayCaster::RaySet>
PinholeCamRayCaster::getRaySet(movements::Pose &sensor_pose)
{
        Ray ray;
        ray.origin = sensor_pose.position;

        boost::shared_ptr<PinholeCamRayCaster::RaySet> ray_set =
                boost::make_shared<PinholeCamRayCaster::RaySet>();
...
boost::shared_ptr<PinholeCamRayCaster::RayDirectionSet>
PinholeCamRayCaster::getRayDirectionSet(movements::Pose &sensor_pose)
{
        boost::shared_ptr<PinholeCamRayCaster::RayDirectionSet> ray_dirs =
                boost::make_shared<PinholeCamRayCaster::RayDirectionSet>();
...
boost::shared_ptr<const PinholeCamRayCaster::RayDirectionSet>
PinholeCamRayCaster::getRelRayDirectionSet() const
{
        return boost::const_pointer_cast<const RayDirectionSet>(
                ray_directions_);
}
