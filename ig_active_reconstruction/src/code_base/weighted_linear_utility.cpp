...
void WeightedLinearUtility::useInformationGain(std::string name,
                                               double weight) {
...
WeightedLinearUtility::getNbv(views::ViewSpace::IdSet &id_set,
  ...
  for (size_t i = 0; i < number_of_threads; ++i) {
    threads.push_back(
        std::thread(&WeightedLinearUtility::getIg, this, std::ref(ig_vector),
                    std::ref(total_multitthread_ig[i]), command,
                    std::ref(id_set), viewspace, i, number_of_threads));
  }
  for (size_t i = 0; i < number_of_threads; ++i) {
    threads[i].join();
    total_ig += total_multitthread_ig[i];
    cout << "ig :" << i << "gain :" << total_multitthread_ig[i] << endl;
  }
      ...
      for (unsigned int i = 0; i < information_gains.size(); ++i) {
        if (information_gains[i].status ==
          ...
          cout << "information_gains[" << i
               << "] :" << information_gains[i].predicted_gain << endl;
