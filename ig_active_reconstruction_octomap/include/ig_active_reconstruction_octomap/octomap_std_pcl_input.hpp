...
public:
  ...
  public:
    bool use_bounding_box; //! Whether incoming pointclouds should be filtered
                           //! or not using the configured bounding box.
                           //! Default:false.
    ::octomap::point3d bounding_box_min_point_m; //! Defines bounding box
                                                 //! minimum. Points with
                                                 //! smaller coordinates are
                                                 //! discarded, default: lowest
                                                 //! double possible [m].
    ::octomap::point3d bounding_box_max_point_m; //! Defines bounding box
                                                 //! maximum. Points with larger
                                                 //! coordinates are discarded,
                                                 //! default: largest double
                                                 //! possible [m].
    double max_sensor_range_m; //! Maximal range for integrating sensor data
                               //! [m]. Anything exceeding this distance will be
                               //! dropped. For negative values, it is ignored.
                               //! Default: -1.
  };
