...
namespace octomap
{
/*! Calculates occlusion distances for a given distance along rays behind
 * points.
 */
template <class TREE_TYPE, class POINTCLOUD_TYPE>
class RayOcclusionCalculator
    : public OcclusionCalculator<TREE_TYPE, POINTCLOUD_TYPE>
{
      public:
        struct Options {
              public:
                Options(double occlusion_update_dist_m)
                    : occlusion_update_dist_m(occlusion_update_dist_m){};

              public:
                double occlusion_update_dist_m; //! Max distance behind points
                                                //! along ray for which an
                                                //! occlusion will be calculated
                                                //! [m].
        };
      ...
      protected:
        double occlusion_update_dist_m_; //! Max distance behind points along
                                         //! ray for which an occlusion will be
                                         //! calculated [m].
};
} // namespace octomap
} // namespace world_representation
