...
namespace octomap {
/*! Templated class that implements the area factor presented by Vasquez-Gomez
 * et al. in their paper "Volumetric next best view planning for 3d object
 * reconstruction with positioning error, Journal of Advanced Robotic Systems,
 * vol.11, 2014"
 */
...
public:
  ...
  /*! Returns the information gain calculated for all data added so far (For all
   * rays).
   */
  virtual GainType getInformation();

  /*! Clears all ray-specific data, next data will be considered part of new
   * ray.
   */
  virtual void makeReadyForNewRay();

  /*! Tells the metric that the current computation has ended and any next call
   * will refer to a new
...
protected:
  ...
  /*! Helper function to speed up calculations: Precomputes needed coefficients
   * for a given alpha.
   */
  void setCoefficients(double alpha, double &a_f1, double &b_f1, double &a_f2,
                       double &b_f2, double &c_f2, double &d_f2);

  /*! Area factor function as defined in paper with precalculated coefficients
   * for occupied voxel percentage.
   * @return The area factor.
   */
  double areaFactorOccupied(double percentage);

  /*! Area factor function as defined in paper with precalculated coefficients
   * for occplane voxel percentage.
   * @return The area factor.
   */
...
private:
  ...
  uint64_t
      voxel_count_; //! Voxels integrated for the information gain calculation.

  bool no_known_voxel_so_far_;
  bool previous_voxel_free_;
  bool ray_is_already_registered_; //! To ensure that every ray is only
                                   //! registered once.
