...
namespace octomap {
/*! Templated class that implements the "rear side entropy information gain" as
 * presented in the
 ...
 * likelihood and restricts the area that is considered to such on the rear side
 * of already observed surfaces.
 */
...
private:
  ...
  uint64_t
      current_ray_voxel_count_; //! Number of voxels integrated for calculation.

  double p_vis_;                //! Visibility likelihood for next voxel.
  bool previous_voxel_unknown_; //! Whether or not the previous voxel was
                                //! unknown.
};
}
}
