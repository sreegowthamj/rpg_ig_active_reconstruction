...
public:
   ...
   * Registers an information gains with optional constructor parameters that
   ...
   *
   * @tparam IG_METRIC_TYPE Type of the information gain that is to be created.
   * It must be templated on the TREE_TYPE, which will be automatically matched
   * to the one of the IgCalculator object.
   * @param args (variadic) Constructor arguments to the created IG_METRIC_TYPE,
  ...
  /*! Registers an information gain with an optional Utils type constructor
   * parameter that will then be available for calculations. It must take the
   * TREE_TYPE as its only template argument which is being set automatically.
   */
