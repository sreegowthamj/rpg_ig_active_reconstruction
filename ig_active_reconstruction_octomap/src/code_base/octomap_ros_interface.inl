...
CSCOPE::RosInterface(Config config)
    : nh_(config.nh), world_frame_name_(config.world_frame_name)
{
        voxel_map_publisher_ = nh_.advertise<visualization_msgs::MarkerArray>(
...
void CSCOPE::publishVoxelMap()
{
        ...
        for (typename TREE_TYPE::iterator it = this->link_.octree->begin(),
                                          end = this->link_.octree->end();
             it != end; ++it) {
                ...
                if (this->link_.octree->isNodeOccupied(*it)) {
                        ...
                        occupiedNodesVis.markers[idx].points.push_back(
                                cubeCenter);
                }
        }
