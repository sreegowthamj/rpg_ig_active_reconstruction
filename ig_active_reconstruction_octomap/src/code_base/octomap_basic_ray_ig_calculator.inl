...
void CSCOPE::availableIgMetrics(std::vector<MetricInfo> &available_ig_metrics)
{
        ...
        for (it = this->ig_factory_.begin(), end = this->ig_factory_.end();
             it != end; ++it) {
...
void CSCOPE::availableMapMetrics(std::vector<MetricInfo> &available_map_metrics)
{
        ...
        for (it = this->mm_factory_.begin(), end = this->mm_factory_.end();
             it != end; ++it) {
        ...
        RayCastSettings &setting)
{
        ...
        double max_range =
                (setting.max_ray_depth > 0) ? setting.max_ray_depth : 0.0;
