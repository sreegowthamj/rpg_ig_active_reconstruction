...
CSCOPE::BasicRayIgCalculator(Config config)
    : config_(config), ray_caster_(config.ray_caster_config) {}
...
CSCOPE::computeViewIg(IgRetrievalCommand &command,
  ...
  if (command.path.empty()) {
    ...
    for (unsigned int i = 0;
         i < (!command.metric_ids.empty() ? command.metric_ids.size()
  ...
  boost::shared_ptr<RayCaster::RaySet> ray_set =
  ...
  ray_cast_settings.max_ray_depth =
      config_.ray_caster_config
          .max_ray_depth_m; // command.config.max_ray_depth;
  ...
  typename std::vector<boost::shared_ptr<InformationGain<TREE_TYPE>>>::iterator
      ig_it = ig_set.begin();
...
void CSCOPE::availableIgMetrics(std::vector<MetricInfo> &available_ig_metrics) {
  ...
  for (it = this->ig_factory_.begin(), end = this->ig_factory_.end(); it != end;
       ++it) {
...
void CSCOPE::availableMapMetrics(
  ...
  for (it = this->mm_factory_.begin(), end = this->mm_factory_.end(); it != end;
       ++it) {
...
void CSCOPE::calculateIgsOnRay(
  ...
  bool found_endpoint = this->link_.octree->castRay(
      origin, direction, end_point, true, max_range); // ignore unknown cells

  if (!found_endpoint) // this is necessary for occlusion based metrics but not
                       // for the others. Excluding it leads to a great speed
                       // up.
  {
    end_point =
        origin +
  ...
  if (found_endpoint) {
    ...
    if (this->link_.octree->coordToKeyChecked(end_point, end_key)) {
      typename TREE_TYPE::NodeType *traversedVoxel =
          this->link_.octree->search(end_key);
