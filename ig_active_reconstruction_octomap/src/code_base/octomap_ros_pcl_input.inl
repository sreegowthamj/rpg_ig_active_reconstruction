...
CSCOPE::RosPclInput(
  ...
  pcl_subscriber_ =
      nh_.subscribe("pcl_input", 10, &CSCOPE::insertCloudCallback, this);
  pcl_input_service_ =
      nh_.advertiseService("pcl_input", &CSCOPE::insertCloudService, this);
}
...
TEMPT
void CSCOPE::insertCloudCallback(
...
void CSCOPE::issueInputDoneSignals() {
  BOOST_FOREACH (boost::function<void()> &call, signal_call_stack_) { call(); }
}
...
void CSCOPE::insertCloud(POINTCLOUD_TYPE &pointcloud) {
  ...
  try {
    tf_listener_.lookupTransform(world_frame_name_, pointcloud.header.frame_id,
                                 ros::Time(0), sensor_to_world_tf);
  } catch (tf::TransformException &ex) {
    ROS_ERROR_STREAM("RosPclInput<TREE_TYPE,POINTCLOUD_TYPE>::Transform error "
                     "of sensor data: "
