...
CSCOPE::Config::Config()
    : use_bounding_box(false),
      bounding_box_min_point_m(-std::numeric_limits<double>::max(),
                               -std::numeric_limits<double>::max(),
                               -std::numeric_limits<double>::max()),
      bounding_box_max_point_m(std::numeric_limits<double>::max(),
                               std::numeric_limits<double>::max(),
                               std::numeric_limits<double>::max()),
      max_sensor_range_m(-1) {}

TEMPT
CSCOPE::StdPclInput(Config config) : config_(config) {}

TEMPT
void CSCOPE::push(
    const Eigen::Transform<double, 3, Eigen::Affine> &sensor_to_world,
    POINTCLOUD_TYPE &pc) {
  ...
  if (config_.use_bounding_box) {
    ...
    pass_z.setFilterLimits(config_.bounding_box_min_point_m.z(),
                           config_.bounding_box_max_point_m.z());

    pcl::PassThrough<typename POINTCLOUD_TYPE::PointType> pass_x;
    pass_x.setFilterFieldName("x");
    pass_x.setFilterLimits(config_.bounding_box_min_point_m.x(),
                           config_.bounding_box_max_point_m.x());

    pcl::PassThrough<typename POINTCLOUD_TYPE::PointType> pass_y;
    pass_y.setFilterFieldName("y");
    pass_y.setFilterLimits(config_.bounding_box_min_point_m.y(),
                           config_.bounding_box_max_point_m.y());
  ...
  for (size_t i = 0; i < valid_indices.size(); ++i) {
    ...
    point3d point(pc_cpy->points[valid_indices[i]].x,
                  pc_cpy->points[valid_indices[i]].y,
                  pc_cpy->points[valid_indices[i]].z);
    // maxrange check
    point3d curr_ray = point - sensor_origin;

    if (i % 100 == 0)
      std::cout << "\nBuilding iterator set: " << i << "/"
                << valid_indices.size();

    if ((config_.max_sensor_range_m < 0.0) ||
        (curr_ray.norm() <= (config_.max_sensor_range_m + 0.000001))) {
      // free cells
      if (this->link_.octree->computeRayKeys(sensor_origin, point,
                                             key_ray_temp)) {
      ...
      point3d new_end =
          sensor_origin + curr_ray.normalized() * config_.max_sensor_range_m;
      if (this->link_.octree->computeRayKeys(sensor_origin, new_end,
                                             key_ray_temp)) {
        free_cells.insert(key_ray_temp.begin(), key_ray_temp.end());
      }
    }
  ...
  // mark free cells only if not seen occupied in this cloud - attention: voxels
  // may already exist even though no actual measurement has yet been received
  // at their position (e.g. if their occlusion distance was calculated) - need
  // to check hasMeasurement()!
  size_t count = 0;
    ...
    if (occupied_cells.find(*it) == occupied_cells.end()) {
      ...
      if (voxel == NULL) {
        ...
        if (!voxel->hasMeasurement()) {
          float logOddsFirstMiss =
              ::octomap::logodds(this->link_.octree->config().miss_probability);
  ...
  for (KeySet::iterator it = occupied_cells.begin(), end = free_cells.end();
       it != end; ++it) {
    if (count++ % 100 == 0)
      std::cout << "\nInserting occupied: " << count << "/"
                << occupied_cells.size();
    ...
    if (voxel == NULL) {
      ...
      if (!voxel->hasMeasurement()) {
        float logOddsFirstHit =
            ::octomap::logodds(this->link_.octree->config().hit_probability);
