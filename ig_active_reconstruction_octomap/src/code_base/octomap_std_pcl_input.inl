        ...
        POINTCLOUD_TYPE &pc)
{
        ...
        for (size_t i = 0; i < valid_indices.size(); ++i) {
                ...
                if ((config_.max_sensor_range_m < 0.0)
                    || (curr_ray.norm()
                        <= (config_.max_sensor_range_m + 0.000001))) {
                        // free cells
                        if (this->link_.octree->computeRayKeys(
                                    sensor_origin, point, key_ray_temp)) {
                        ...
                        point3d new_end =
                                sensor_origin
                                + curr_ray.normalized()
                                          * config_.max_sensor_range_m;
                        if (this->link_.octree->computeRayKeys(
                                    sensor_origin, new_end, key_ray_temp)) {
                                free_cells.insert(key_ray_temp.begin(),
                                                  key_ray_temp.end());
                        }
                }
        ...
        // mark free cells only if not seen occupied in this cloud - attention:
        // voxels may already exist even though no actual measurement has yet
        // been received at their position (e.g. if their occlusion distance was
        // calculated) - need to check hasMeasurement()!
        size_t count = 0;
        for (KeySet::iterator it = free_cells.begin(), end = free_cells.end();
             it != end; ++it) {
                if (count++ % 1000 == 0)
                        std::cout << "\nInserting free: " << count << "/"
                                  << free_cells.size();
                ...
                if (occupied_cells.find(*it) == occupied_cells.end()) {
                        ...
                        if (voxel == NULL) {
                                voxel = this->link_.octree->updateNode(*it,
                                                                       false);
                                ...
                                if (!voxel->hasMeasurement()) {
                                        ...
                                        this->link_.octree->updateNode(*it,
                                                                       false);
                                }
                        }
                }
        ...
        for (KeySet::iterator it = occupied_cells.begin(),
             ...
             it != end; ++it) {
                ...
                if (voxel == NULL) {
                        ...
                        if (!voxel->hasMeasurement()) {
                                float logOddsFirstHit = ::octomap::logodds(
                                        this->link_.octree->config()
                                                .hit_probability);
