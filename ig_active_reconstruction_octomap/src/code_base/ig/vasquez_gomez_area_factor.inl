...
CSCOPE::VasquezGomezAreaFactorIg(Config utils)
  ...
  setCoefficients(utils_.vasquez_config.p_occupied_des_, a_f1_occu_, b_f1_occu_,
                  a_f2_occu_, b_f2_occu_, c_f2_occu_, d_f2_occu_);
  setCoefficients(utils_.vasquez_config.p_occplane_des_, a_f1_occp_, b_f1_occp_,
                  a_f2_occp_, b_f2_occp_, c_f2_occp_, d_f2_occp_);
}

TEMPT
std::string CSCOPE::type() { return "VasquezGomezAreaFactorIg"; }
...
void CSCOPE::includeEndPointMeasurement(typename TREE_TYPE::NodeType *node) {
  if (!includeMeasurement(node)) // if ray wasn't registered to the last voxel
                                 // on it, it's an unmarked one
  {
    unobserved_count_ += 1;
  }
}
...
bool CSCOPE::includeMeasurement(typename TREE_TYPE::NodeType *node) {
  ...
  if (node == NULL) {
    ...
    if (no_known_voxel_so_far_) {
      previous_voxel_free_ = true; // treat unknown at beginning of ray,
                                   // starting from sensor position as p_free
    } else {
      previous_voxel_free_ = false; // treat unknown at beginning of ray,
                                    // starting from sensor position as p_free
    }
  ...
  {
    if (previous_voxel_free_) // occupied voxels should only be registered as
                              // occupied if they are actually expected to be
                              // visible (and not hit from the backside...)
    {
      ++occupied_count_;
    }
    previous_voxel_free_ = false;
...
double CSCOPE::areaFactorOccupied(double percentage) {
  ...
  if (percentage <= utils_.vasquez_config.p_occupied_des_) {
    ...
    return a_f2_occu_ * x3 + b_f2_occu_ * x2 + c_f2_occu_ * percentage +
           d_f2_occu_;
  }
}
