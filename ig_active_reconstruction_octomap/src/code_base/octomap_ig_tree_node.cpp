...
IgTreeNode::IgTreeNode()
    : ::octomap::OcTreeNode(), occ_dist_(-1),
      max_dist_(std::numeric_limits<double>::max()), has_no_measurement_(false)
{
}
...
{
        return rhs.value == value && rhs.occ_dist_ == occ_dist_
               && rhs.has_no_measurement_ == has_no_measurement_;
}
...
double IgTreeNode::getMeanChildLogOdds() const
{
        ...
        if (children != NULL) {
                for (unsigned int i = 0; i < 8; i++) {
                        IgTreeNode *child =
                                static_cast<IgTreeNode *>(children[i]);
                        if (child != NULL) {
